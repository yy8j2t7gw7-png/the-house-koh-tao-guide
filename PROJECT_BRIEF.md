...
Current release: v5.11.36.

The live release prioritizes the operational guest journey and AI Concierge. v5.11.36 is a narrow corrective release built directly from deployed v5.11.35 after real production testing found two snorkeling-answer failures. Independent local-information turns may no longer accept an accidental `welcome` match caused by the short **hi** trigger appearing inside **which**. When an independent snorkeling-information question has approved retrieved records, those records are now authoritative and the answer is produced deterministically without relying on model compliance; actionable snorkeling booking intent is unchanged. The v5.11.35 current-turn detour architecture, approved Koh Tao activities/bars/beaches/cafés/restaurants/shopping retrieval with `EXPLORE_ENABLED=false`, Bamboo priority, Mae Haad/Sairee distance guidance, missing-supply alerts, AI-first routine contact rules, five reviewed Meta action templates, universal House Maps link and every Room 11, lost-key, emergency, booking, luggage, passport, Airbnb, Admin, privacy and security safeguard remain authoritative.
