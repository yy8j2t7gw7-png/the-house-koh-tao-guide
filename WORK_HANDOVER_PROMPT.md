...
The current release is v5.11.41, a narrow mobile/emergency UX correction on the already-pushed v5.11.40 human-routing release. Property/fire emergencies expose **Call The House Emergency Support** without revealing the configured responder name or telephone number; fire keeps Koh Tao Rescue first and deliberate urgent-alert confirmation separate. The exact follow-up **Do you have a emergency contact I can call** is deterministic and alert-free. On mobile, once conversation starts, the seven quick actions compact into three columns so the transcript gets more space, and drag-to-dismiss is disabled so ordinary chat scrolling cannot move or close the entire Concierge sheet. v5.11.40 remains the persistent-human correction for **I need to talk to a human you can not help me** during normal service hours. v5.11.39 cleaning/state behavior remains authoritative, including deterministic clean up/cleanup, retained cleaning workflow, bare-hour continuation, automatic structured service submission, fresh reload state, access-aware greeting and context-free send-request blocking. All earlier Wi-Fi, snorkeling, French Kiss Divers, lost-key, emergency, booking, luggage, passport, Airbnb, Admin, Meta and security boundaries remain authoritative. The five newer human-friendly Meta replacements remain inactive until separately approved for activation.
...
Deploy v5.11.41 without changing `EXPLORE_ENABLED=false`, secrets, recipient groups, webhook settings, the currently active five action-template mappings, `WHATSAPP_STAFF_ACTIONS_ENABLED=true`, passport/Airbnb configuration or `SPARE_KEY_CODES`. Smoke-test a fire/property emergency and confirm **Call The House Emergency Support** is present without exposing any private responder name/number; confirm **Do you have a emergency contact I can call** returns emergency-call actions without creating an alert; confirm the mobile chat has more transcript space after conversation start; and confirm repeated message scrolling never moves or dismisses the sheet. Then spot-check v5.11.40 persistent human routing and the v5.11.39 cleaning/state regression. Do not activate pending replacement Meta templates automatically.
