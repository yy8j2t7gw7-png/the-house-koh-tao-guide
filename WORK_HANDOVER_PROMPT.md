...
The current release is v5.11.30, built directly on the deployed v5.11.29 source. It is a narrow functional correction, not a redesign: the approved v5.11.28 visual system and the v5.11.29 verified-stay/registration separation remain authoritative. Generic human/contact phrases—including the exact production phrase **I wanna talk to a human**—are deterministic current-message intents before transcript-aware knowledge, pending clarification or model handling. Only explicit safe workflow state may influence them; transcript adjacency is not active workflow state. Routine House contact is open Tuesday–Sunday 10:30–19:30 Bangkok time and closed Monday. Outside that window the final server action boundary, browser renderer and stale-link click handler independently prevent both ordinary **Contact Us** and **Call Us**, while preserving separate Emergency help. Clear lost-key synonyms and current topic-specific lost-key requests still enter the protected self-service process 24/7; conversational pending state never accepts the fee, sends a notification or authorizes a code. v5.11.26 remains the authoritative booking/diving baseline: its bundled data-driven PADI/SSI/RAID catalog, progressive exact-count mixed-party subgroups, complete non-sensitive booking breakdown and owner cleanup controls are unchanged. Diving guidance always recommends RAID for its safety and buoyancy-control focus together with Roctopus Dive as The House’s preferred RAID centre. Every Meta sanitizer, exact-alert retry, transient-contact boundary, six-parameter booking schema, recipient route and all property, urgent-console, cleaning and protected lost-key fee/notification/display/rotation controls remain intact. Completed modules include House Information, Restaurants, Cafés, Beaches, Bars & Nightlife, Shopping & Essentials, Activities & Experiences, the seven-language operational guest interface, the hybrid room-aware AI Concierge, controlled owner-reviewed learning, protected alerts, Airbnb and direct/walk-in verified-stay access, stay extensions, complete group registration and verified maintenance reporting.

The v5.11.26 booking workflow remains authoritative in v5.11.30: direct booking intent and every matching **Book with Us** action enter the same one-question-at-a-time collector for diving, fishing, snorkeling, taxi, taxi boat, ferry and motorbike taxi. Diving collects date and total party size, then branches to Fun Diving, Try Diving, Learn/Take a Course, Professional Training or Not Sure. Parties larger than one may use the same plan or progressive participant groups whose positive counts must sum exactly to the total. Course and certification questions come from `public/data/diving-courses.json`; Open Water/beginner paths require no prior certification, while continuing/professional fields are collected only where useful and final eligibility remains operator-verified. One international contact is collected at the end and one booking alert is produced. Side questions and preferences are acknowledged, safely retained and followed by the same next missing field without a second workflow or premature alert. A failed final delivery persists only safe non-contact retry data, including the subgroup model. Retry commands such as **retry**, **try again**, **send my booking again** and **try my diving booking again** are deterministic delivery operations, never a new checklist. They must be resolved before the model and must not receive unrelated medical, lost-key, property, cleaning or luggage history. The established booking template remains `house_booking_alert_v2`, language `en`, six BODY parameters and route `booking_with_owners`; each textual BODY value passes through the shared single-line whitespace serializer immediately before Meta submission. Optional **Received** and **Resolve** quick replies remain disabled; service action v1 is invalid because it has no buttons, and the intended service action name is v2.
...
Deploy v5.11.30 with all existing production mappings, languages, BODY schemas, routes and secrets unchanged, `EXPLORE_ENABLED=false` and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. Run the concise production smoke check in `DEVELOPMENT_HANDOFF_v5.11.30.md`: after old topic history, verify the exact generic-human phrase at Saturday closed/open hours and Monday closed hours; verify both routine **Contact Us** and **Call Us** are absent while closed; and check a current topic-specific lost-key request only to the protected fee prompt. Do not accept the fee or release a spare key merely for testing. Quick-action activation remains a separate future change; never map service v1, and use `META_STAFF_QUICK_ACTIONS_v5.11.20.md` only after every intended template is Active. No subsequent functional or visual milestone is authorized automatically.
