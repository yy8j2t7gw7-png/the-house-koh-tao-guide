...
The current release is v5.11.34, built directly on deployed v5.11.33. It is a mobile-only Room 11 image-framing hotfix. Inside the existing `@media(max-width:760px)` rule, Room 11 keeps horizontal focus `72%` and changes only vertical focus from `58%` to `100%` so the marked entrance is fully visible. Tablet/desktop framing is unchanged. v5.11.33 remains authoritative for the stable floating Concierge behavior. Below 768 px the Concierge is a stable 148×52 px House-green **💬 AI Concierge** bottom-right pill with the accessible name **Open AI Concierge**, iPhone safe-area offsets and modest bottom clearance. The rejected v5.11.32 scroll-collapse/collision/lift controller has been removed completely, so the launcher does not move while scrolling. When the existing Concierge panel opens, the launcher is hidden and removed from the visible/focus surface; it is restored on close. The approved 232 px mobile Room hero remains unchanged; v5.11.34 uses the Room 11 mobile focal point `72% 100%`. v5.11.31 header-owned language access, card spacing, verified/TM30 wording and **Open Concierge** label remain authoritative. v5.11.30 generic human/contact routing, lost-key pending state and routine Contact/Call gates remain unchanged. Routine House contact is open Tuesday–Sunday 10:30–19:30 Bangkok time and closed Monday; Emergency help and protected 24/7 lost-key self-service remain separate. The owner explicitly confirmed no Admin Dashboard WhatsApp diagnostics change is required in v5.11.33.
...
Deploy v5.11.34 with production logic, mappings, languages, BODY schemas, routes and secrets unchanged, `EXPLORE_ENABLED=false` and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. Run only the real-iPhone Room 11 crop check in `DEVELOPMENT_HANDOFF_v5.11.34.md`: the full marked Room 11 entrance must be visible; desktop/tablet must remain unchanged. No booking, lost-key or WhatsApp functional test is required unless automated regressions fail. Do not begin another release automatically.
