...
First live-regression test the v5.11.10 critical-intent override from a pending luggage contact prompt and confirm the three legal links on representative guest, registration, room and owner-console pages.

Deploy and verify v5.11.10. Confirm the concierge launcher opens on every operational page and that ordinary Contact Us and House-support call actions open the concierge first. Confirm the concise verification copy, luggage and resource guidance, readable maintenance references, then test an all-Thai exemption, a multi-passport foreign/mixed stay, the no-upload in-person passport route and a temporary spare-key code using the verified active room session, two-step fee acceptance and automatic team-message submission. The in-person route must stay locked until an authorized owner confirms both the passport check and the separately completed manual TM30 filing; do not automate the Immigration portal. The verified-stay secrets, Airbnb synchronizer, active listing feeds, cross-room rejection and non-sensitive passport upload are already configured or tested. A confirmed 24/7 property-emergency contact remains outstanding. Transport and other Explore modules remain deferred.
