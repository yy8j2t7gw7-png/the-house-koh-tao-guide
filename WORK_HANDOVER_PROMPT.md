...
The current release is v5.11.22. It adds progressive category-specific booking, deterministic property intelligence, protected 24/7 lost-key self-service and collapsible owner operations over the preserved v5.11.21 baseline. It changes no production Meta mapping, secret, recipient, webhook, emergency route, passport-retention rule or Airbnb synchronization behavior. Completed modules include House Information, Restaurants, Cafés, Beaches, Bars & Nightlife, Shopping & Essentials, Activities & Experiences, the seven-language operational guest interface, the hybrid room-aware AI Concierge, controlled owner-reviewed learning, protected alerts, Airbnb and direct/walk-in verified-stay access, stay extensions, complete group registration and verified maintenance reporting.

For v5.11.22, direct booking intent and every matching **Book with Us** action enter the same one-question-at-a-time collector for diving, fishing, snorkeling, taxi, taxi boat, ferry and motorbike taxi. Supplied fields persist; finite options use buttons; a usable international contact is last; recommendation or information questions remain non-actionable. Routine pests, odors, plumbing, equipment, fixtures, mold/damp and room-condition reports route once to Su plus both owners, while dirty-room language stays in cleaning and serious hazards retain the explicit urgent confirmation boundary. Lost-key self-service is available at both daytime and nighttime: each request starts with no accepted fee, is bound to the active stay/room/session/request, requires explicit 500 THB acceptance and one accepted Su-or-owner notification, and becomes unusable after protected-page display until physical code rotation and authorized reset. Optional **Received** and **Resolve** quick replies remain disabled; service action v1 is invalid because it has no buttons, and the intended service action name is v2.
...
Deploy v5.11.22 with all existing production mappings and secrets unchanged and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. Run the changed-function production smoke matrix in `DEVELOPMENT_HANDOFF_v5.11.22.md`, including all seven bookings, routine/urgent property cases, lost key at 16:00 and 23:00, negative/stale/replay cases, rotation and owner-console collapse behavior. Stop after reporting the result. The next planned development milestone is v5.11.23, a full visual-polish release; do not begin it without a new explicit scope. Quick-action activation remains a separate future change; never map service v1, and use `META_STAFF_QUICK_ACTIONS_v5.11.20.md` only after every intended template is Active.
