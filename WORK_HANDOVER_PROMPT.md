...
Deploy v5.11.26 with all existing production mappings, languages, BODY schemas, routes and secrets unchanged and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. Run the concise mixed-diving and owner-cleanup checks in `DEVELOPMENT_HANDOFF_v5.11.26.md`. Confirm one mixed booking record, three attempts, at least one accepted delivery, an accurate compact subgroup summary, maintenance photo cleanup and diagnostic dismissal without delivery-state mutation. The next planned milestone is v5.11.27 — full visual polish; do not begin it without a new explicit scope. Quick-action activation remains a separate future change; never map service v1, and use `META_STAFF_QUICK_ACTIONS_v5.11.20.md` only after every intended template is Active.
