...
`src/maintenance-api.js` owns verified room-problem reporting and deterministic routine/critical classification. Optional images use the private R2 binding and never enter AI or public assets. The admin side permits open/acknowledged reports to become resolved and permits removal only after resolution; a remaining private image must be deleted from R2 before its report row is removed. `src/whatsapp-alerts.js` owns protected staff delivery; a critical guest reply contact remains transient and is added only to the delivery payload. Its common `textParameters()` boundary serializes every textual Meta BODY value as a trimmed single line with Unicode whitespace runs collapsed to one ordinary space before applying the existing length limit. This applies to every current alert/status template and future enabled action-template BODY without changing schema order or count. Diagnostic visibility is stored separately from alert and delivery truth, so dismissal never marks a request delivered. None of these sensitive operations is delegated to the language model.

Structured booking alerts are stored before outbound delivery so the protected operation is auditable. `public/data/diving-courses.json` and `src/diving-catalog.js` define the current agency/course model; `src/concierge-api.js` stores one booking with exact-count participant groups and `src/whatsapp-alerts.js` independently validates them before building the unchanged six-parameter Meta request. A complete non-sensitive breakdown lives in `concierge_alert_details`, while the outbound template receives a concise 900-character-safe summary. Deduplication carries only code-free delivery counts. An alert with failed attempts and zero acceptances creates a durable contact-free `booking_retry_snapshots` record plus safe `booking_retry_group_details`. Its binding is a one-way hash of the verified reservation, room and protected browser session, and its lifetime is limited by the verified stay/session boundary. Explicit retry is resolved before knowledge or model routing and may deliver only under the original alert ID. The transient contact is used from protected memory or recollected alone after a reload; it is never stored. Any prior accepted delivery suppresses another send. The guest receives success wording only after at least one accepted provider message ID. The protected owner alert joins only sanitized provider diagnostics by alert ID.
