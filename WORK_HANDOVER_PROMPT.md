...
The current release is v5.11.42, a narrow Concierge routing and stay-extension booking release on deployed v5.11.41. Human-contact intent now uses a broad deterministic family so natural wording for a human/person/staff/team/reception/front desk/housekeeper/manager/agent/representative/customer support does not fall through merely because phrasing differs. A first ordinary request remains AI-first; repeated, frustrated, urgent or direct-transfer wording exposes **The House team** Contact Us / Call Us only during Tuesday–Sunday 10:30–19:30 Bangkok service hours, with no private staff identity and no alert. Natural stay-extension wording enters booking kind `stay_extension`, collects additional nights plus an international WhatsApp/phone number, and sends exactly one existing booking alert through `booking_with_owners` to Fah plus both owners. The guest must never be told the extension is confirmed before availability and payment are checked. v5.11.41 emergency-call privacy/mobile stability, v5.11.39 cleaning/state behavior and all earlier security boundaries remain authoritative. The five newer human-friendly Meta replacements remain inactive until separately approved for activation. Airbnb synchronization is unchanged in v5.11.42; a newly observed last-minute booking sync delay is a separate next-release issue.
...
After v5.11.42 is deployed and smoke-tested, address the separate production issue where a last-minute Airbnb booking may not become available to guest-page verification promptly. Investigate the existing Gmail-triggered/iCal synchronization path first, measure where the delay occurs, and implement the smallest reliable fix. Prefer fast booking-email detection plus a bounded forced iCal reconciliation rather than wasteful full-room polling every few minutes. Preserve confirmation-code hashing, room binding, direct/walk-in stays, all privacy/security boundaries and Cloudflare/Apps Script free-tier constraints. Do not change the v5.11.42 human/stay-extension behavior while fixing Airbnb sync.
