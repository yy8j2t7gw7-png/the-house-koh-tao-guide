# TAOEDGE CUMULATIVE HANDOFF — Backend v5.11.82 / Owner App v0.1.26
## Copilot Reliability + Visual Separation Hotfix
...
## 11. Paired Owner App v0.1.26
- Adds a real dimmed scrim behind the floating Copilot panel so the underlying hotel screen is visibly background context rather than appearing to be Copilot options.
- Background becomes non-interactive while Copilot is expanded; tap outside closes the panel.
- Uses a stronger elevated sheet surface, border/shadow and iOS-style drag handle.
- Keeps the app behind visible, but establishes an unmistakable foreground/background hierarchy.
- Improves the generic Copilot failure wording so it accurately describes live hotel-information loading rather than implying an undefined reconnect state.
- Diagnostics expects backend v5.11.82+.
- Inventory is still PENDING and must not be falsely presented as shipped.
- Voice interaction is approved future scope: push-to-talk + spoken replies first, live conversation later, under the same Copilot permissions/action safety.
