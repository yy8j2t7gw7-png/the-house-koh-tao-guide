...
The current release is v5.11.35, built directly on deployed v5.11.34. A clear current-turn local-information question takes precedence over stale cleaning, booking, luggage or ordinary support context; the exact question alone drives targeted retrieval/model input, while a valid pending workflow is normalized and returned for later resumption. The Concierge retrieves approved activities, bars, beaches, cafés, restaurants and shopping records internally even though `EXPLORE_ENABLED=false`. General drinks/nightlife guidance leads with Bamboo Beach Bar, but explicit late-night or food requirements may select a better approved fit. House-specific guidance places Mae Haad Beach about 200 metres down the road and Sairee Beach at roughly a 20-minute walk. Natural missing towel, toilet-paper and soap statements immediately use the existing protected service-alert path; informational supply questions create no alert. Routine page-level House calls have been removed: **Contact Us** and **Book with Us** open the Concierge, and generic human contact exposes routine call/WhatsApp only after an AI-first attempt and a persistent request during Tuesday–Sunday 10:30–19:30 Bangkok hours. Emergency and protected 24/7 lost-key actions remain separate. All five reviewed Meta staff-action templates are enabled with exact fail-closed mappings and **Received** then **Resolve**; the accidental buttonless service action v1 remains invalid. The single working House Maps URL remains unchanged after successful mobile retesting. v5.11.34's mobile-only Room 11 `72% 100%` focal point, v5.11.33's stable 148×52 px **💬 AI Concierge** pill, launcher-hidden-while-open behavior and every passport, Airbnb, admin, security and operational safeguard remain authoritative.

The v5.11.26 booking workflow remains authoritative in v5.11.35: direct booking intent and every matching **Book with Us** action enter the same one-question-at-a-time collector for diving, fishing, snorkeling, taxi, taxi boat, ferry and motorbike taxi. Diving collects date and total party size, then branches to Fun Diving, Try Diving, Learn/Take a Course, Professional Training or Not Sure. Parties larger than one may use the same plan or progressive participant groups whose positive counts must sum exactly to the total. Course and certification questions come from `public/data/diving-courses.json`; Open Water/beginner paths require no prior certification, while continuing/professional fields are collected only where useful and final eligibility remains operator-verified. One international contact is collected at the end and one booking alert is produced. Side questions and preferences are acknowledged, safely retained and followed by the same next missing field without a second workflow or premature alert. A failed final delivery persists only safe non-contact retry data, including the subgroup model. Retry commands such as **retry**, **try again**, **send my booking again** and **try my diving booking again** are deterministic delivery operations, never a new checklist. They must be resolved before the model and must not receive unrelated medical, lost-key, property, cleaning or luggage history. The established buttonless booking template remains the rollback path. The active reviewed booking-action template is `house_booking_alert_actions_v1`, generic English (`en`), with the same six BODY parameters and `booking_with_owners` route; each textual BODY value passes through the shared single-line whitespace serializer immediately before Meta submission. **Received** and **Resolve** quick replies use only the authorized command and opaque alert ID.
...
Deploy v5.11.35 with `EXPLORE_ENABLED=false`, the exact five reviewed action-template mappings and `WHATSAPP_STAFF_ACTIONS_ENABLED=true`; do not change secrets, recipient groups, webhook settings, passport/Airbnb configuration or `SPARE_KEY_CODES`. Run the affected-behavior smoke test in `RELEASE_NOTES_v5.11.35.md`: missing towels, topic switching after cleaning, Mae Haad/Sairee distance, Bamboo-first drinks, dataset-backed dining, AI-first human escalation, Concierge-first public CTAs, one action alert with both quick replies, the retained universal House Maps link and Emergency Help. Do not begin another release automatically.
