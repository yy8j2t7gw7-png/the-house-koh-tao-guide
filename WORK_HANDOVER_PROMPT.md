...
The current release is v5.11.21. It is a narrow functional correction over v5.11.20: cleaning clock preferences are validated against the actual Bangkok date and time, robust natural fishing and snorkeling wording enters structured booking collection, and lost-key responses use hospitality language without exposing delivery or code-release internals. It changes no production Meta mapping, secret, recipient, webhook, emergency route, passport-retention rule, Airbnb synchronization behavior or visual design. Completed modules include House Information, Restaurants, Cafés, Beaches, Bars & Nightlife, Shopping & Essentials, Activities & Experiences, the seven-language operational guest interface, the hybrid room-aware AI Concierge, controlled owner-reviewed learning, protected alerts, Airbnb and direct/walk-in verified-stay access, stay extensions, complete group registration and verified maintenance reporting.

For v5.11.21, a past same-day cleaning time, a closed-hour time or a Monday preference creates no alert and keeps collection active; a valid correction creates exactly one Su-plus-owner service alert. Explicit future dates remain attached to their own requested day, while `now` and `ASAP` retain their immediate-request behavior. Direct wording such as **I wanna go fishing**, **I’d like to book fishing** and **Take us snorkeling**, plus every category-specific **Book with Us** action, enters structured booking collection; information questions remain non-actionable. Lost-key authorization, routing, fee, protected delivery and code-rotation gates are unchanged, but guest copy no longer describes those implementation details. Optional **Received** and **Resolve** quick replies remain disabled; service action v1 is invalid because it has no buttons, and the intended service action name is v2.
...
Deploy v5.11.21 with all existing production mappings and secrets unchanged and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. Run only the short changed-function smoke test: reject a past same-day cleaning time and submit one valid correction; verify **I wanna go fishing** and the informational **Book with Us** action both enter structured fishing collection; then verify the office-hours lost-key success response is natural hospitality copy with one dedicated alert and no technical terms. Stop after reporting the result. Do not begin the deferred v5.11.22 visual-polish work without a new explicit scope. Quick-action activation remains a separate future change; never map service v1, and use `META_STAFF_QUICK_ACTIONS_v5.11.20.md` only after every intended template is Active.
