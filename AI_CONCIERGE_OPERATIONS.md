...
The v5.11.24 concierge combines six layers:
...
Failed WhatsApp booking delivery is visible in the protected owner alert itself. The card shows only safe operational evidence: channel/provider, `booking_with_owners` route, selected template, template language, attempted and accepted counts, HTTP status when present, actual sanitized provider error classification/message and Bangkok timestamp. It never shows recipient numbers, the guest contact, template parameter values, tokens, request authorization, stay credentials or key-box data. The production booking mapping remains `house_booking_alert_v2`, generic English (`en`) and six ordered BODY parameters; v5.11.24 does not invent a Meta error or change that mapping without provider evidence.
