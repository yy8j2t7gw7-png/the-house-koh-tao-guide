...
Current release: v5.11.37 (narrow production correction on deployed v5.11.36: strong/direct human and housekeeper contact can reach Su during open hours without guessing an exact persistence phrase; a cancelled fire does not contaminate a clearly unrelated next contact turn while genuine fire continuation stays safety-first; natural stained-linen wording enters the existing cleaning collector and `now`/`ASAP` completes the same request; explicit French Kiss Divers preference is retained before or during a diving collector without promising availability; authorized Wi-Fi-password questions use deterministic approved knowledge so the numeric password is not removed by generic contact-number sanitization. Preserve the v5.11.36 snorkeling fix and all Meta, lost-key, emergency, booking, luggage, passport, Airbnb, Admin, privacy and security boundaries; do not activate the pending replacement Meta templates in this release).
