...
All seven progressive booking categories share one authoritative collector and Bangkok date parser. During an active workflow, distinguish side questions and preferences from the currently requested field, acknowledge them naturally without promising third-party availability, retain only sanitized notes and continue with the same next missing field. Invalid fields must receive a concise reason before the question is asked again. Diving course names, aliases and prerequisite-question flags come from the bundled catalog, not duplicated branching literals. Parties may have one plan or exact-count participant subgroups; every count must be positive, never exceed the remaining party and sum exactly to the total before contact collection. Fun Diving requires useful sanitized current-certification information; beginner courses do not, while continuing/professional courses request a current level only where the catalog marks it useful. Final eligibility remains with the operator. Always pair the RAID safety/buoyancy recommendation with Roctopus Dive, and never imply that Roctopus issues an explicitly requested PADI/SSI certification. A valid international contact replaces a rejected local attempt without clearing fields. If protected delivery has attempts but no acceptance, store a durable contact-free `delivery_failed` retry snapshot bound to the verified reservation, room, protected browser session and original alert ID, including only safe subgroup data. Resolve explicit retry before knowledge/model routing and exclude broad transcript context. The snapshot must yield to every unrelated intent, reuse no other alert and never authorize a send after any accepted delivery. If transient contact is gone, ask only for it again. Never create a second alert or show success without a provider message ID.
...
Never put key-box codes, readable Airbnb or direct-stay confirmation codes, private guest tokens or API credentials in public source or structured content. Follow `SECURE_24_HOUR_LOST_KEY_ACCESS.md` for the protected spare-key flow. Real key-box codes belong only in the encrypted `SPARE_KEY_CODES` Worker secret. Automatic release must fail closed unless a verified active stay session bound to that reservation and room, a short-lived single-use request bound to the current protected session, deliberate guest acceptance of the 500 THB fee for that request, automatic Su-and-owner notification with at least one confirmed WhatsApp API submission, and rotation state all pass. The guest does not re-enter the stay code for this operation. A protected reset clears only the room rotation lock and requires one of two explicit modes: controlled administrative test with existing code retained, or completed physical key-box rotation. Store distinct code-free audit activity and preserve all historical used-request markers. Direct/walk-in codes must be generated server-side, shown only in the authorized creation response and persisted only as an HMAC hash. Extensions may move checkout later without changing the reservation identity or bypassing the verified-session limit.
...
- validate booking side-question continuity, shared date feedback, flexible certification, property category/issue-instance isolation and forced-open unresolved urgent owner operations
- validate both deliberate spare-key reset modes, truthful code-free activity, cancelled confirmation, historical replay rejection and new-request fee reset
