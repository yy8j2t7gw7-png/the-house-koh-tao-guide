...
First live-regression test the v5.11.13 safety boundary. Confirm figurative/slang phrases such as `I am dying for love`, `bloody hell` and `I am burning inside` create no alert. Confirm a medical message shows Rescue/1669 guidance but creates no House alert until **Send urgent alert** is pressed; **Cancel** must create none. Confirm flooding or dangerous property language still enters its explicit confirmation flow. Then send one message once and verify only one Concierge response appears.

Deploy and verify v5.11.13. On a verified active room after hours, open lost-key help, confirm the simplified 500 THB checkbox flow, successful protected notification before code release and the post-release rotation lock with its Concierge contact action. Recheck the v5.11.12 complete-field luggage workflow and universal contact redaction, plus the concierge launcher, owner-inclusive routing, all-Thai exemption, multi-passport foreign/mixed stay, no-upload in-person passport route, legal links and maintenance reporting. The in-person route must stay locked until an authorized owner confirms both the passport check and the separately completed manual TM30 filing; do not automate the Immigration portal. The verified-stay secrets, Airbnb synchronizer, active listing feeds, cross-room rejection and non-sensitive passport upload are already configured or tested. A confirmed 24/7 property-emergency contact remains outstanding. Transport and other Explore modules remain deferred.
