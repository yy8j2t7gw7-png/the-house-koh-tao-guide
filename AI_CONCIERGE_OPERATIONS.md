...
The v5.11.26 concierge combines six layers:
...
Actionable House booking requests use deterministic, category-specific workflows. Direct conversational intent such as “I want to go fishing”, “I would like to snorkel”, “I wanna go fishing”, “I’d like to book fishing” and “Take us snorkeling” starts structured collection immediately, as does the matching **Book with Us** action. The grammar distinguishes these requests from genuine information questions such as “What fishing trips do you offer?”, which remain informational until the guest chooses the booking action. Diving first collects the preferred start/diving date and total party size, then asks for Fun Diving, Try Diving, Learn/Take a Course, Professional Training or Not Sure. Parties larger than one choose the same plan or progressive subgroups whose positive integer counts must allocate the total exactly. Course branches use the bundled `public/data/diving-courses.json` PADI/SSI/RAID catalog, ask certification only where useful and retain the complete non-sensitive subgroup breakdown in the alert. RAID is always recommended for its safety and buoyancy-control focus together with Roctopus Dive as The House’s preferred RAID centre; explicit PADI/SSI choices remain valid and are checked with an appropriate provider without implying that Roctopus issues those certifications. A rejected local contact remains redacted and cannot erase or poison collected fields; its valid international replacement completes the same request. Fishing collects date, guest count and fishing style; snorkeling collects date, guest count and trip type; taxi and motorbike taxi collect date, time, pickup, destination and passenger count; taxi/longtail boat additionally collects one-way or return; ferry collects date, origin, destination and traveler count, with time accepted when supplied. Every category requires one usable international reply contact at the end. The final alert boundary validates the structure again, each new request starts clean and recommendation-only questions remain informational. The flow asks only for missing fields, preserves supplied data, never requests passport data for ferry bookings and never starts a personal Fah chat. During any active category, a side question or preference is acknowledged, retained as a sanitized note and followed by the same next missing field; third-party availability is never promised. The shared Bangkok parser accepts dotted, slashed, dashed, named and relative dates and explains past or unparseable input. Valid requests route through `booking_with_owners`. If every delivery attempt fails, the completed safe fields enter `delivery_failed` rather than ordinary collection and a contact-free retry snapshot—including safe subgroup data—is stored under the same alert ID. The snapshot is bound by a one-way hash to the verified reservation, room and protected browser session and expires at the protected stay/session boundary. Bar, checkout, Wi-Fi, property and other unrelated intents route normally with no resend. Explicit retry detection runs before knowledge, broad history and model routing, so stale medical, lost-key or property context cannot enter the operation. A retry reuses the same alert only when attempts exist and accepted deliveries remain zero; an accepted alert is never resent. If the raw contact disappeared at the privacy boundary, every safe field remains and the guest is asked only for an international contact. Guest wording states that current availability, price, schedule, prerequisites where relevant and payment must be checked before confirmation.

Failed WhatsApp booking delivery is visible in the protected owner alert itself. The card shows only safe operational evidence: channel/provider, `booking_with_owners` route, selected template, template language, attempted and accepted counts, HTTP status when present, actual sanitized provider error classification/message and Bangkok timestamp. It never shows recipient numbers, the guest contact, template parameter values, tokens, request authorization, stay credentials or key-box data. Production captured Meta HTTP `400`, error `132018`, proving that a textual BODY value contained forbidden newline/tab characters or excessive spaces. v5.11.25 corrects this only at the common outbound `textParameters()` serializer: every Unicode whitespace run becomes one ordinary space before the existing length limit. The production booking mapping remains `house_booking_alert_v2`, generic English (`en`) and six ordered BODY parameters; no template, language, route, recipient or Meta configuration changes.

The protected owner console keeps operational state separate from cleanup visibility. Open or acknowledged maintenance reports may be resolved; only resolved reports may be removed, and any remaining private photo object must be deleted successfully first. Individual failed-delivery diagnostics may be dismissed without changing the parent alert or its accepted/failed counts. All diagnostics for one alert may be cleared only after that alert is resolved. These actions use deliberate custom-dialog confirmation and retain only minimal contact-free, code-free audit references and Bangkok timestamps.
