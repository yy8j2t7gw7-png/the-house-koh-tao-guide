...
The v5.11.20 concierge combines six layers:
...
Routine requests for toilet paper, soap, towels and room cleaning are deterministic service operations and take priority over older ordinary collection workflows. Natural room-condition wording such as “my room is dirty” is treated as the same cleaning operation rather than left to model interpretation. These requests never require a guest phone number. Housekeeping operates Tuesday–Sunday from 10:30 through 19:29 Bangkok time and is unavailable all day Monday. Supply requests are alerted immediately. During open hours the guest receives the existing 30-minute general House call fallback; outside those hours the response names the actual next opening and does not show a routine call fallback. Sunday requests at or after 19:30 and all Monday requests correctly point to Tuesday at 10:30. Cleaning first collects a preferred clock time, `now` or `ASAP`; only then is one alert created with that preference, and the guest is told that the requested time is not confirmed until staff can accommodate it. The Concierge never asks the guest to submit the same request manually. These alerts use `support_with_owners` and never expose Su’s personal details.

Lost-key assistance has its own deterministic policy and never falls through to service, urgent or booking classification. A current verified room-bound active stay is required, but passport upload or registration completion is not an authorization gate. An unverified request creates no alert and exposes no code. During office hours, a verified request creates only the dedicated `lost_key` alert to `lost_key_team`. After hours, the verified guest deliberately accepts the 500 THB fee and the Worker must receive at least one accepted protected team notification before returning the key-box code to the protected page. Codes never enter Concierge history, alerts, delivery payloads, logs or diagnostics. The room rotation lock and current session, stay and room checks prevent prior state from authorizing a new release.

Actionable House booking requests use deterministic, category-specific workflows. Direct first-person intent such as “I want to go fishing” or “I would like to snorkel” starts structured collection, as does the matching **Book with Us** action. Diving retains its preferred-date, diver-count, experience/course, conditional certification/course-detail and international-contact gates. Fishing collects date, guest count and fishing style; snorkeling collects date, guest count and trip type; taxi and motorbike taxi collect date, time, pickup, destination and passenger count; taxi/longtail boat additionally collects one-way or return; ferry collects date, origin, destination and traveler count, with time accepted when supplied. Every category requires a usable international reply contact. The final alert boundary validates the structure again, each new request starts clean and recommendation-only questions remain informational. The flow asks only for missing fields, preserves supplied data, never requests passport data for ferry bookings and never starts a personal Fah chat. Valid requests route through `booking_with_owners`, and guest wording always states that current availability and price must be checked and that the booking remains unconfirmed until payment is complete.
