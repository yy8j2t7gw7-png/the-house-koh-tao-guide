...
The current release is v5.11.32, built directly on deployed v5.11.31. It is a mobile presentation correction, not an operational workflow change. Below 768 px the idle launcher is a 148×52 px House-green **💬 AI Concierge** pill with the accessible name **Open AI Concierge**. Six cumulative downward-scroll pixels collapse it to 52×52 px; 650/220 ms direction-sensitive debounces, 10/16 px collision/release gaps and 90 ms throttled collision checks prevent nervous behavior. Visible controls are measured together; the full pill collapses before overlap and the compact bubble moves upward to the nearest safe right-aligned position when necessary. Right/bottom safe-area offsets and modest bottom clearance remain. Desktop retains **✦ Concierge**. The mobile Room hero is 232 px; Room 11 uses a protected-page `72% 58%` focal position and a left-side overlay so the actual marked entrance remains visible. v5.11.31 header-owned language access, card spacing, verified/TM30 wording and **Open Concierge** label remain authoritative. v5.11.30 generic human/contact routing, lost-key pending state and routine Contact/Call gates remain unchanged. Routine House contact is open Tuesday–Sunday 10:30–19:30 Bangkok time and closed Monday; Emergency help and protected 24/7 lost-key self-service remain separate. The v5.11.26 booking/diving baseline, RAID/Roctopus policy, Meta sanitizer, retry, contact, recipient, property, cleaning and secure lost-key controls remain intact.

The v5.11.26 booking workflow remains authoritative in v5.11.32: direct booking intent and every matching **Book with Us** action enter the same one-question-at-a-time collector for diving, fishing, snorkeling, taxi, taxi boat, ferry and motorbike taxi. Diving collects date and total party size, then branches to Fun Diving, Try Diving, Learn/Take a Course, Professional Training or Not Sure. Parties larger than one may use the same plan or progressive participant groups whose positive counts must sum exactly to the total. Course and certification questions come from `public/data/diving-courses.json`; Open Water/beginner paths require no prior certification, while continuing/professional fields are collected only where useful and final eligibility remains operator-verified. One international contact is collected at the end and one booking alert is produced. Side questions and preferences are acknowledged, safely retained and followed by the same next missing field without a second workflow or premature alert. A failed final delivery persists only safe non-contact retry data, including the subgroup model. Retry commands such as **retry**, **try again**, **send my booking again** and **try my diving booking again** are deterministic delivery operations, never a new checklist. They must be resolved before the model and must not receive unrelated medical, lost-key, property, cleaning or luggage history. The established booking template remains `house_booking_alert_v2`, language `en`, six BODY parameters and route `booking_with_owners`; each textual BODY value passes through the shared single-line whitespace serializer immediately before Meta submission. Optional **Received** and **Resolve** quick replies remain disabled; service action v1 is invalid because it has no buttons, and the intended service action name is v2.
...
Deploy v5.11.32 with production logic, mappings, languages, BODY schemas, routes and secrets unchanged, `EXPLORE_ENABLED=false` and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. Run only the real-iPhone smoke check in `DEVELOPMENT_HANDOFF_v5.11.32.md`: idle pill, downward collapse, safe re-expansion, collision avoidance over important actions, Room 11 marked-entrance crop and opening the same Concierge from both pill states. No booking or lost-key functional test is required unless automated regressions fail. Do not begin another release automatically.
