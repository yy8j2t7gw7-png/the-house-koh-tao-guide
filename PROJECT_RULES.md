...
Current release: v5.11.36 (narrow production correction on deployed v5.11.35: independent local-information questions cannot be hijacked by an accidental `welcome` match from the short **hi** trigger inside another word such as **which**; independent snorkeling-information questions with approved retrieved records are answered directly from project knowledge without relying on model compliance; actionable snorkeling bookings remain protected structured workflows. All v5.11.35 local-guide, workflow-preservation, missing-supply, AI-first contact, Meta quick-action, House Maps, Room 11, lost-key, emergency, booking, luggage, passport, Airbnb, Admin, privacy and security rules remain authoritative).
