...
The current release is v5.11.25. It is a narrow Meta template-parameter sanitization hotfix built directly on deployed v5.11.24. Production owner diagnostics conclusively captured Meta HTTP `400`, error `132018`: a textual template parameter contained forbidden newline/tab characters or excessive consecutive spaces. The final common `textParameters()` boundary in `src/whatsapp-alerts.js` now converts every Unicode whitespace run to one ordinary space, trims and applies the existing length limit. This covers service, luggage, booking, urgent, lost-key, status and any future enabled action-template BODY without changing template names, languages, BODY counts/order, routes, recipients or Meta configuration. Every v5.11.24 exact-alert retry, contact-free snapshot, contact-only reload recovery, unrelated-intent routing, sanitized diagnostic and provider-normalization behavior remains intact, as do all earlier property, urgent-console, cleaning and 24/7 lost-key corrections. Completed modules include House Information, Restaurants, Cafés, Beaches, Bars & Nightlife, Shopping & Essentials, Activities & Experiences, the seven-language operational guest interface, the hybrid room-aware AI Concierge, controlled owner-reviewed learning, protected alerts, Airbnb and direct/walk-in verified-stay access, stay extensions, complete group registration and verified maintenance reporting.

The v5.11.24 booking workflow remains authoritative: direct booking intent and every matching **Book with Us** action enter the same one-question-at-a-time collector for diving, fishing, snorkeling, taxi, taxi boat, ferry and motorbike taxi. Side questions and preferences are acknowledged, safely retained and followed by the same next missing field without a second workflow or premature alert. Supplied fields persist; finite options use buttons; a usable international contact is last; recommendation or information questions remain non-actionable. A failed final delivery persists only safe non-contact retry data. Retry commands such as **retry**, **try again**, **send my booking again** and **try my diving booking again** are deterministic delivery operations, never a new checklist. They must be resolved before the model and must not receive unrelated medical, lost-key, property, cleaning or luggage history. The established booking template remains `house_booking_alert_v2`, language `en`, six BODY parameters and route `booking_with_owners`; each textual BODY value now passes through the shared single-line whitespace serializer immediately before Meta submission. Routine property and secure lost-key behavior remains as documented for v5.11.23. Optional **Received** and **Resolve** quick replies remain disabled; service action v1 is invalid because it has no buttons, and the intended service action name is v2.
...
Deploy v5.11.25 with all existing production mappings, languages, BODY schemas, routes and secrets unchanged and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. Run the single fresh non-sensitive diving-booking smoke test in `DEVELOPMENT_HANDOFF_v5.11.25.md`: exactly one alert, three attempts, at least one accepted delivery, one received booking message and the normal pending confirmation. If Meta rejects it, capture only the new sanitized diagnostic and stop. The next planned development milestone is v5.11.26 — full visual polish; do not begin it without a new explicit scope. Quick-action activation remains a separate future change; never map service v1, and use `META_STAFF_QUICK_ACTIONS_v5.11.20.md` only after every intended template is Active.
