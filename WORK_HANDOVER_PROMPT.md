...
Guests may continue to use ordinary WhatsApp handoffs. v5.9.0 contains a separate official WhatsApp Business Platform adapter for protected staff alerts, including role-based recipients, signed acknowledgement and urgent/critical escalation. Automatic spare-key release fails closed until the Meta account, Utility template and encrypted urgent recipients in `WHATSAPP_ALERT_OPERATIONS.md` are configured.

v5.9.0 uses one permanent page for each active Room 1–6 and 8–11. Room 7 is inactive. Reservation data is synchronized with `airbnb-sync/Code.gs`; readable confirmation codes are HMAC-hashed before storage. A verified guest can create a private single-use form automatically for each non-Thai overnight guest, or record the exemption when every overnight guest is a Thai national. Read `AIRBNB_AUTOMATION_SETUP.md` and `PASSPORT_DATA_OPERATIONS.md`. Passport content must never enter the model, learning queue, public assets, Airbnb messages or WhatsApp. The manual-details alternative remains blocked until the authoritative TM30 field list is supplied.
...
Deploy and verify v5.9.0, configure the verified-stay secrets, install the Airbnb synchronizer, activate the prepared scheduled arrival messages, and test cross-room rejection, non-sensitive passport upload, Thai exemption and a temporary spare-key code with automatic team-message submission. A confirmed 24/7 property-emergency contact remains outstanding. Transport and other Explore modules remain deferred.
