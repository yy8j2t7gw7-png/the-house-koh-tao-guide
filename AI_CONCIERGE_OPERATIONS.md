...
The v5.11.16 concierge combines six layers:
...
Routine requests for toilet paper, soap, fresh or clean towels and room cleaning are deterministic service operations. Between 10:30 and 19:30 Bangkok time the guest receives an immediate confirmation and a 30-minute general House call fallback. After hours the request is still alerted immediately, but the guest is told it will be handled after 10:30 the following morning. These alerts use `support_with_owners` and never expose Su’s personal details.
