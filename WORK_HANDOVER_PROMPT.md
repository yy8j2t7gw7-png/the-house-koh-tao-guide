...
Deploy v5.11.33 with production logic, mappings, languages, BODY schemas, routes and secrets unchanged, `EXPLORE_ENABLED=false` and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. Run only the real-iPhone smoke check in `DEVELOPMENT_HANDOFF_v5.11.33.md`: stable AI Concierge pill, no scroll movement, launcher absent while chat is open, launcher restored on close, Room 11 marked-entrance crop and header-owned language access. No booking, lost-key or WhatsApp functional test is required unless automated regressions fail. Do not begin another release automatically.
