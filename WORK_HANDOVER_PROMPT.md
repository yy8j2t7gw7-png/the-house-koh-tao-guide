...
The current release is v5.11.20. It is a narrow functional-fix release over production v5.11.19: natural cleaning requests now complete their own protected submission, direct activity intent reliably enters structured booking collection, and lost-key authorization now uses current active-stay verification independently of passport completion. It changes no production Meta mapping, secret, recipient, webhook, emergency route, passport-retention rule, Airbnb synchronization behavior or visual design. Completed modules include House Information, Restaurants, Cafés, Beaches, Bars & Nightlife, Shopping & Essentials, Activities & Experiences, the seven-language operational guest interface, the hybrid room-aware AI Concierge, controlled owner-reviewed learning, protected alerts, Airbnb and direct/walk-in verified-stay access, stay extensions, complete group registration and verified maintenance reporting.

For v5.11.20, wording such as **my room is dirty** enters the deterministic cleaning workflow, asks only for a missing preferred time and then sends exactly one Su-plus-owner service alert containing that preference. Direct wording such as **I want to go fishing** and every category-specific **Book with Us** action enters structured booking collection; information questions remain non-actionable. Lost-key requests cannot use browser-only fallback or generic-service routing. An unverified stay produces no alert or code; a verified office-hours request creates one dedicated lost-key alert; a verified after-hours request retains the 500 THB, accepted-notification-before-code and rotation gates. Passport completion is separate from active-stay verification. Optional **Received** and **Resolve** quick replies remain disabled; service action v1 is invalid because it has no buttons, and the intended service action name is v2.
...
Room cleaning—including natural dirty-room wording—must collect a preferred time before one service alert is created. The accepted preference may be a clock time, `now` or `ASAP`, but the response must say that availability may vary and must not tell the guest to submit the request manually. Routine call fallback appears only while housekeeping is open Tuesday–Sunday, 10:30–19:30 Bangkok time. Direct booking intent and **Book with Us** actions must preserve the service category and enter the structured flow; information-only questions must not create alerts. Fishing, snorkeling, taxi, taxi/longtail boat, ferry and motorbike taxi each have category-specific required fields in addition to the existing diving gates. No booking is confirmed until availability is checked and payment is received. Ferry workflows must never ask for passport data.
...
Deploy v5.11.20 with all existing production mappings and secrets unchanged and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. Smoke-test only the three changed functions: complete **my room is dirty** with a preferred time and verify one service alert; enter a fishing flow both from direct intent and **Book with Us** and verify structured collection; then test unverified, verified office-hours/passport-pending and verified after-hours lost-key paths, including the fee, notification-before-code and rotation gates. Do not begin visual polish automatically. Quick-action activation remains a separate future change; never map service v1, and use `META_STAFF_QUICK_ACTIONS_v5.11.20.md` only after every intended template is Active.
