...
import {
  ...
  beds24DirectStayFastInventorySyncEnabled,
...
test("v5.11.58 expense submission is granular: owners/managers have it, staff require an explicit grant", () => {
  ...
  assert.deepEqual(MOBILE_DELEGATABLE_PERMISSIONS.staff, ["finance.expense_submit", "copilot.use", "inventory.view", "inventory.adjust", "inventory.local_purchase", "staff.schedule_view"]);
...
test("v5.11.77 Listings & Rates mobile route contract is explicit, aliased and trailing-slash safe", async () => {
  ...
  assert.match(source, /backendVersion: "5\.11\.86"/);
...
test("v5.11.77 Direct Stay fast inventory close remains independent from the full Channel Manager", async () => {
  const originalFetch = globalThis.fetch;
  const calls = [];
  const events = [];
  const roomMap = JSON.stringify(Object.fromEntries(Array.from({ length: 11 }, (_, index) => [String(84001 + index), String(index + 1)])));
  const store = {
    async getMessagingProviderState() { return { value: { accessToken: "access" }, expiresAt: new Date(Date.now() + 3_600_000).toISOString() }; },
    async recordReservationDistributionEvent(event) { events.push(event); return { ok: true }; }
  };
  const env = {
    BEDS24_CHANNEL_MANAGER_ENABLED: "false",
    BEDS24_DIRECT_STAY_PROTECTION_ENABLED: "true",
  ...
  assert.equal(beds24DirectStayFastInventorySyncEnabled(env), true);
  assert.equal(beds24ChannelManagerEnabled(env), false);
  globalThis.fetch = async (url, options = {}) => {
    calls.push({ url: String(url), method: options.method || "GET", body: options.body || "" });
    if ((options.method || "GET") === "POST") return new Response(JSON.stringify([{ success: true }]), { status: 200, headers: { "content-type": "application/json" } });
    return new Response(JSON.stringify({ data: [
      { roomId: 84011, date: "2026-09-18", availability: 0 },
      { roomId: 84011, date: "2026-09-19", availability: 0 }
    ] }), { status: 200, headers: { "content-type": "application/json" } });
  };
  try {
    const result = await accelerateBeds24DirectStayClose(env, store, {
      reservationId: "stay_fast_close_001", externalBookingId: "99118", room: "11",
      checkInDate: "2026-09-18", checkOutDate: "2026-09-20"
    });
    assert.equal(result.ok, true);
    assert.equal(result.status, "confirmed_closed");
    const post = calls.find((call) => call.method === "POST");
    assert.ok(post);
    assert.match(post.url, /inventory\/rooms\/calendar/);
    assert.deepEqual(JSON.parse(post.body), [{ roomId: 84011, calendar: [{ from: "2026-09-18", to: "2026-09-19", inventory: 0 }] }]);
    assert.ok(events.some((event) => event.eventType === "inventory_close_requested"));
    assert.ok(events.some((event) => event.eventType === "inventory_close_confirmed" && event.status === "confirmed"));
  } finally {
    globalThis.fetch = originalFetch;
  }
});

test("v5.11.77 cancellation reopening never overwrites another local reservation", async () => {
  const originalFetch = globalThis.fetch;
  let fetches = 0;
  const events = [];
  const env = {
  ...
  const store = {
    async findStayOverlap() { return { id: "stay_other_001", room: "4" }; },
    async recordReservationDistributionEvent(event) { events.push(event); return { ok: true }; }
  };
  globalThis.fetch = async () => { fetches += 1; throw new Error("provider must not be called when local conflict exists"); };
  try {
    const result = await reassertBeds24DirectStayOpenIfSafe(env, store, {
      reservationId: "stay_cancelled_001", externalBookingId: "99119", room: "4",
      checkInDate: "2026-09-20", checkOutDate: "2026-09-22"
    });
    assert.equal(result.ok, true);
    assert.equal(result.status, "blocked_by_other_reservation");
    assert.equal(fetches, 0);
    assert.ok(events.some((event) => event.eventType === "inventory_reopen_skipped"));
  } finally {
    globalThis.fetch = originalFetch;
  }
});

test("v5.11.77 Direct Stay edits check newly-added provider availability before updating Beds24", async () => {
  const originalFetch = globalThis.fetch;
  const calls = [];
  const roomMap = JSON.stringify(Object.fromEntries(Array.from({ length: 11 }, (_, index) => [String(85001 + index), String(index + 1)])));
  const store = { async getMessagingProviderState() { return { value: { accessToken: "access" }, expiresAt: new Date(Date.now() + 3_600_000).toISOString() }; } };
  const env = { BEDS24_DIRECT_STAY_PROTECTION_ENABLED: "true", BEDS24_REFRESH_TOKEN: "refresh", STAY_TOKEN_PEPPER: "pepper", BEDS24_ROOM_MAP: roomMap };
  globalThis.fetch = async (url, options = {}) => {
    calls.push({ url: String(url), method: options.method || "GET", body: options.body || "" });
    if ((options.method || "GET") === "POST") return new Response(JSON.stringify([{ success: true }]), { status: 200, headers: { "content-type": "application/json" } });
    return new Response(JSON.stringify({ data: [{ roomId: 85004, date: "2026-09-22", availability: 1 }] }), { status: 200, headers: { "content-type": "application/json" } });
  };
  try {
    const result = await updateBeds24HouseDirectBooking(env, store, {
      externalBookingId: "551122",
      current: { room: "4", checkInDate: "2026-09-20", checkOutDate: "2026-09-22" },
      next: { room: "4", checkInDate: "2026-09-20", checkOutDate: "2026-09-23" }
    });
    assert.equal(result.ok, true);
    assert.match(calls[0].url, /inventory\/rooms\/availability/);
    const post = calls.find((call) => call.method === "POST");
    assert.ok(post);
    assert.match(post.url, /bookings/);
    assert.deepEqual(JSON.parse(post.body), [{ id: 551122, roomId: 85004, arrival: "2026-09-20", departure: "2026-09-23" }]);
  } finally {
    globalThis.fetch = originalFetch;
  }
});

test("v5.11.77 direct synchronization retries run on the minute while full Channel Manager stays off", async () => {
  const [indexSource, wrangler, storeSource] = await Promise.all([
    readFile(new URL("../src/index.js", import.meta.url), "utf8"),
    readFile(new URL("../wrangler.jsonc", import.meta.url), "utf8"),
    readFile(new URL("../src/concierge-store.js", import.meta.url), "utf8")
  ]);
  assert.match(indexSource, /cron === "\*\/1 \* \* \* \*"[\s\S]*processBeds24ChannelManagerRetries/);
  assert.match(wrangler, /"BEDS24_CHANNEL_MANAGER_ENABLED": "false"/);
  assert.match(wrangler, /"BEDS24_DIRECT_STAY_FAST_INVENTORY_SYNC_ENABLED": "true"/);
