...
const STAY_SUPPORT_REQUEST_WORDS = /(?:\b(?:(?:i|we)\s+(?:need|want|would\s+like)|(?:can|could)\s+(?:i|we|you)\s+(?:have|get|bring|send|provide)|please\s+(?:bring|send|provide))\b[^.?!]{0,60}\b(?:toilet\s+paper|soap|towels?)\b|\b(?:toilet\s+paper|soap|towels?)\s+please\b|\b(?:please\s+)?clean\s+(?:my|our|the)\s+room\b|\b(?:i|we)\s+(?:need|want|would\s+like)\s+(?:a\s+)?(?:room\s+cleaning|housekeeping|clean\s*up|cleanup)\b)/i;
