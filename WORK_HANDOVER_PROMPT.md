...
Deploy v5.11.36 without changing `EXPLORE_ENABLED=false`, secrets, recipient groups, webhook settings, the exact five reviewed action-template mappings, `WHATSAPP_STAFF_ACTIONS_ENABLED=true`, passport/Airbnb configuration or `SPARE_KEY_CODES`. Smoke-test **which beach is good for snorkeling**, **which beach is best for snorkeling** and **is there good snorkeling** first; each must return approved local snorkeling recommendations, no generic welcome, no learning-gap handoff, no alert and no booking workflow. Then spot-check one actionable snorkeling booking to confirm the protected collector is unchanged. Do not begin another release automatically.
