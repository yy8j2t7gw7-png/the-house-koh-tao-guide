...
The v5.11.18 concierge combines six layers:
...
Actionable luggage-storage requests use a deterministic multi-turn workflow. Arrival/departure context, requested time, bag count and a usable international reply contact are mandatory before an alert is created. The browser carries an explicit validated workflow object containing only the operational fields and sanitized notes; it never contains a contact. This keeps the same request intact when a local-format number is rejected and the guest supplies a corrected international number. The final alert-creation boundary validates the structured fields again and refuses incomplete submissions even if a conversational or model layer produces an incorrect handoff. Every new actionable request starts clean after a completed request. Informational questions do not enter the workflow. A raw contact is redacted immediately from the visible Concierge bubble for every request type, held only in browser memory for the active request, sent separately from normal conversation history, and included only in the transient protected staff-delivery payload. A vague or specific urgent incident interrupts and clears this ordinary workflow before its message can be consumed as luggage data.

Routine requests for toilet paper, soap, towels and room cleaning are deterministic service operations and take priority over older ordinary collection workflows. They never require a guest phone number. Between 10:30 and 19:30 Bangkok time the guest receives an immediate confirmation and a 30-minute general House call fallback. After hours the request is still alerted immediately, and the guest is told it has been recorded, will be handled after 10:30 the following morning and does not need to be requested again. These alerts use `support_with_owners` and never expose Su’s personal details.

Actionable diving requests use a separate structured workflow. A booking alert cannot be created until the request contains a preferred date, diver count, a Fun/Open Water/Advanced Open Water/other-course choice, the named other course when applicable, certification level for Fun Diving and an international reply contact. The final alert boundary validates the structure again. Each new booking starts clean; recommendation-only questions remain informational. **Book with Us** opens this Concierge flow and never starts a personal Fah chat. Valid requests route through `booking_with_owners`, and guest wording always states that the booking remains unconfirmed until availability and payment are complete.
