...
- A verified guest may declare that every overnight guest is Thai, or declare the complete number of non-Thai adults and children in a foreign/mixed group. Guest self-service for every declared non-Thai overnight guest—not only the person who made the booking—is secure passport image upload only, one private form per required passport. In-person passport verification is a staff-authorized exception available only through authenticated Owner Admin when a guest cannot or will not use secure upload. Private guest information remains locked until all uploads are received or an authorized owner confirms the exceptional in-person passport check and manual registration are complete. Do not infer nationality from chat or other data.
- Before any passport or Thai-ID evidence has been uploaded, a verified guest may correct an accidentally selected Thai-only vs foreign/mixed guest type without repeating stay verification. The abandoned pending branch must be reset to `not_started` and every unused upload link from that branch must be invalidated. Once document evidence exists, or a staff-authorized in-person registration state has begun/completed, guest self-service may not switch guest type; correction requires Owner Admin review.
...
- The secure verification page must remain concise and action-led while retaining the stay-code instruction, Thai exemption, complete non-Thai group requirement, TM30 purpose, private handling and 14-day deletion. Guest self-service must not expose an in-person passport choice; any in-person exception remains Owner Admin-only.
...
The hybrid, room-aware AI Concierge, full operational translation path, controlled owner-review workflow, protected staff-alert channel, verified-stay automation, direct/walk-in stay creation, stay extensions and verified maintenance reporting are implemented. Production key release is fail-closed until the reservation sync, key-code secret and official urgent WhatsApp recipients are configured and verified. Room 7 is enabled for the guest guide and direct-stay testing but remains excluded from Airbnb synchronization until its real listing is active. Explore remains deferred, and Transport remains research-blocked until the required authoritative source is supplied.
