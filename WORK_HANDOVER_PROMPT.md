...
Deploy v5.11.27 with all existing production mappings, languages, BODY schemas, routes and secrets unchanged and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. Run only the concise six-step visual smoke check in `DEVELOPMENT_HANDOFF_v5.11.27.md`; do not deliberately release a spare key for visual testing. Quick-action activation remains a separate future change; never map service v1, and use `META_STAFF_QUICK_ACTIONS_v5.11.20.md` only after every intended template is Active. No subsequent functional or visual milestone is authorized automatically.
