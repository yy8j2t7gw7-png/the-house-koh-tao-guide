...
The v5.11.19 concierge combines six layers:
...
Routine requests for toilet paper, soap, towels and room cleaning are deterministic service operations and take priority over older ordinary collection workflows. They never require a guest phone number. Housekeeping operates Tuesday–Sunday from 10:30 through 19:29 Bangkok time and is unavailable all day Monday. Supply requests are alerted immediately. During open hours the guest receives the existing 30-minute general House call fallback; outside those hours the response names the actual next opening and does not show a routine call fallback. Sunday requests at or after 19:30 and all Monday requests correctly point to Tuesday at 10:30. Cleaning first collects a preferred clock time, `now` or `ASAP`; only then is one alert created, and the guest is told that the requested time is a preference rather than a confirmed appointment. These alerts use `support_with_owners` and never expose Su’s personal details.

Actionable House booking requests use deterministic, category-specific workflows. Diving retains its preferred-date, diver-count, experience/course, conditional certification/course-detail and international-contact gates. Fishing collects date, guest count and fishing style; snorkeling collects date, guest count and trip type; taxi and motorbike taxi collect date, time, pickup, destination and passenger count; taxi/longtail boat additionally collects one-way or return; ferry collects date, origin, destination and traveler count, with time accepted when supplied. Every category requires a usable international reply contact. The final alert boundary validates the structure again, each new request starts clean and recommendation-only questions remain informational. **Book with Us** opens the matching Concierge flow and never starts a personal Fah chat. Valid requests route through `booking_with_owners`, and guest wording always states that the booking remains unconfirmed until availability and payment are complete.
