...
The current release is v5.11.17. It is the evidence-based correction for the confirmed v5.11.16 Meta HTTP 404 / code `132001` template-translation rejection. Completed modules include House Information, Restaurants, Cafés, Beaches, Bars & Nightlife, Shopping & Essentials, Activities & Experiences, the seven-language operational guest interface, the hybrid room-aware AI Concierge, controlled owner-reviewed learning, protected alerts, Airbnb and direct/walk-in verified-stay access, stay extensions, complete group registration and verified maintenance reporting.
...
Deploy v5.11.17 without altering any Meta template definition, Cloudflare mapping or secret. First repeat the verified Room 11 request `I need fresh towels` and confirm `house_service_alert_v3` delivers with language `en`, one BODY component and five text parameters to Su plus both owners. Then live-test luggage, booking, confirmed urgent property, verified lost-key, `RECEIVED`/`ACK`/`RESOLVE`, diagnostics and the guest-footer Admin Login link on desktop and mobile. Meta code `132001` must not recur.
