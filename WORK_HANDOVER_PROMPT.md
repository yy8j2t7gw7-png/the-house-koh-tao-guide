...
The current release is v5.11.18. It preserves the live-passed v5.11.17 Meta template-language correction and adds explicit luggage workflow state, routine-housekeeping precedence and meaningful urgent clarification. Completed modules include House Information, Restaurants, Cafés, Beaches, Bars & Nightlife, Shopping & Essentials, Activities & Experiences, the seven-language operational guest interface, the hybrid room-aware AI Concierge, controlled owner-reviewed learning, protected alerts, Airbnb and direct/walk-in verified-stay access, stay extensions, complete group registration and verified maintenance reporting.

For v5.11.18, pending luggage fields survive a rejected local-format contact and attach to the corrected international contact without entering ordinary history or storage. Verified-room towels, soap, toilet paper and cleaning requests always bypass older ordinary collection state and create a service alert without asking for contact details. Vague urgent wording starts a high-priority clarification state with no send action; the actual follow-up incident is classified before confirmation, and generic incident summaries fail closed at submission.
...
Deploy v5.11.18 without altering any Meta template definition, Cloudflare mapping or secret. First repeat the verified Room 11 requests for fresh towels, soap and toilet paper and confirm `house_service_alert_v3` delivers with language `en`, one BODY component and five text parameters to Su plus both owners without a contact prompt. Then live-test the local-to-international luggage correction, vague-to-specific urgent clarification, booking, verified lost key, `RECEIVED`/`ACK`/`RESOLVE`, diagnostics and the guest-footer Admin Login link on desktop and mobile. Meta code `132001` must not recur.
