...
The current release is v5.11.24. It is a narrow booking retry/delivery-state correction built directly on the deployed v5.11.23 baseline. Explicit retry is recognized before knowledge retrieval, broad transcript context, model/general fallback or a new progressive booking. A durable contact-free snapshot preserves completed safe fields and binds the original failed alert to the verified reservation, room and protected browser session. A valid retry reuses the exact alert ID only when attempts exist and accepted deliveries are zero; an accepted alert is never resent. If the transient contact has disappeared after a reload or privacy boundary, the Concierge asks only for the international contact. Unrelated bar, check-out and property intents route normally. Owner alert cards expose only sanitized real Meta delivery evidence, and provider wording such as “or with Master Divers would be even better” normalizes to Master Divers. It changes no production Meta mapping, secret, recipient, webhook, emergency route, passport-retention rule or Airbnb synchronization behavior and preserves every v5.11.23 booking, property, urgent-console, cleaning and 24/7 lost-key correction. Completed modules include House Information, Restaurants, Cafés, Beaches, Bars & Nightlife, Shopping & Essentials, Activities & Experiences, the seven-language operational guest interface, the hybrid room-aware AI Concierge, controlled owner-reviewed learning, protected alerts, Airbnb and direct/walk-in verified-stay access, stay extensions, complete group registration and verified maintenance reporting.

For v5.11.24, direct booking intent and every matching **Book with Us** action still enter the same one-question-at-a-time collector for diving, fishing, snorkeling, taxi, taxi boat, ferry and motorbike taxi. Side questions and preferences are acknowledged, safely retained and followed by the same next missing field without a second workflow or premature alert. Supplied fields persist; finite options use buttons; a usable international contact is last; recommendation or information questions remain non-actionable. A failed final delivery persists only safe non-contact retry data. Retry commands such as **retry**, **try again**, **send my booking again** and **try my diving booking again** are deterministic delivery operations, never a new checklist. They must be resolved before the model and must not receive unrelated medical, lost-key, property, cleaning or luggage history. The established booking template remains `house_booking_alert_v2`, language `en`, six BODY parameters and route `booking_with_owners`; real provider failure evidence appears only in the protected owner console. Routine property and secure lost-key behavior remains as documented for v5.11.23. Optional **Received** and **Resolve** quick replies remain disabled; service action v1 is invalid because it has no buttons, and the intended service action name is v2.
...
Deploy v5.11.24 with all existing production mappings and secrets unchanged and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. Run only the short failed-booking retry, alert-bound owner diagnostic, unrelated-intent and Master Divers smoke matrix in `DEVELOPMENT_HANDOFF_v5.11.24.md`. Stop after reporting the result. The next planned development milestone is v5.11.25 — full visual polish; do not begin it without a new explicit scope. Quick-action activation remains a separate future change; never map service v1, and use `META_STAFF_QUICK_ACTIONS_v5.11.20.md` only after every intended template is Active.
