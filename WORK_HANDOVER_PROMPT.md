...
Deploy v5.11.31 with all production logic, mappings, languages, BODY schemas, routes and secrets unchanged, `EXPLORE_ENABLED=false` and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. On a real iPhone, run only the focused visual review in `DEVELOPMENT_HANDOFF_v5.11.31.md`: confirm the unverified Emergency CTA and all verified Room content can scroll clear of the compact launcher; verify the Room-location image, navigation cards, Finding Room sequence, **Open Concierge**, Google Maps and footer; and confirm the language control never floats independently over content. No functional booking or lost-key smoke test is required unless automated regressions fail. Do not begin another release automatically.
