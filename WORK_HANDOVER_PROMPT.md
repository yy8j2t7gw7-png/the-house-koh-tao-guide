...
The current release is v5.11.43, a production-critical synchronization, Meta-template and passport-registration release on deployed v5.11.42. The Airbnb synchronizer now checks recent Airbnb host mail every five minutes, immediately posts trustworthy room/listing/code/date records to the existing protected reservation-sync endpoint with `complete:false`, and reconciles all ten active private iCal feeds at least hourly even when no email change is detected. The complete 24-hour audit remains the only absence-based cancellation path. The five newly approved human-friendly Meta action templates are active with visible **Received** / **Resolved** buttons while their signed payload commands remain internal `RECEIVED` / `RESOLVE`. Secure passport Option 2 is active through the existing private single-use link and collects exactly passport number, full name, birthday, nationality, gender/sex as shown on the passport, and phone/WhatsApp number; those values are stored only in private `PASSPORT_UPLOADS` storage and never enter Concierge, learning data, WhatsApp or ordinary operational records. v5.11.42 broad human-contact routing and stay extension, v5.11.41 emergency-call/mobile stability, v5.11.39 cleaning/state behavior and all earlier security boundaries remain authoritative.
...
After v5.11.43 is deployed and smoke-tested, keep the next release narrow and based only on new production findings. First verify the five-minute Apps Script trigger, fast-path timestamp and hourly calendar timestamp in the existing standalone Airbnb sync project; verify all five new Meta action templates and their Received/Resolved lifecycle in production; and verify both passport image and manual-details submissions with fictional test data plus secure deletion. Airbnb scheduled-message timing/content remains a separate external Airbnb dashboard task and should not be mixed into code unless explicitly requested.
