...
The current release is v5.11.23. It is a narrow conversation/state correction built directly on the completed and deployed v5.11.22 baseline. Active bookings now acknowledge side questions and preferences without losing the one authoritative workflow, the shared Bangkok date parser accepts common dotted/slashed/dashed/named/relative formats with useful validation feedback, Fun Diving certification is flexible sanitized free text, property issue detail is isolated by category and issue instance, unresolved urgent owner-console work cannot be manually hidden, and spare-key rotation locks have truthful controlled-test and physical-rotation reset paths. It changes no production Meta mapping, secret, recipient, webhook, emergency route, passport-retention rule or Airbnb synchronization behavior. Completed modules include House Information, Restaurants, Cafés, Beaches, Bars & Nightlife, Shopping & Essentials, Activities & Experiences, the seven-language operational guest interface, the hybrid room-aware AI Concierge, controlled owner-reviewed learning, protected alerts, Airbnb and direct/walk-in verified-stay access, stay extensions, complete group registration and verified maintenance reporting.

For v5.11.23, direct booking intent and every matching **Book with Us** action still enter the same one-question-at-a-time collector for diving, fishing, snorkeling, taxi, taxi boat, ferry and motorbike taxi. Side questions and preferences are acknowledged, safely retained and followed by the same next missing field without a second workflow or premature alert. Supplied fields persist; finite options use buttons; a usable international contact is last; recommendation or information questions remain non-actionable. A failed final booking delivery becomes an explicit-retry snapshot: unrelated new intents route normally and never resend it, while a deliberate retry can reuse the completed safe fields, transient contact and same alert ID. Routine pests, odors, plumbing, equipment, fixtures, mold/damp and room-condition reports route to Su plus both owners with clean category/issue-instance state, while dirty-room language stays in cleaning and serious hazards retain the explicit urgent confirmation boundary. Lost-key self-service remains available at both daytime and nighttime: each request starts with no accepted fee, is bound to the active stay/room/session/request, requires explicit 500 THB acceptance and one accepted Su-or-owner notification, and becomes unusable after protected-page display. A protected owner may then clear only the rotation lock through an explicitly confirmed controlled-test reset or after a real physical rotation; neither path revives the historical request. Optional **Received** and **Resolve** quick replies remain disabled; service action v1 is invalid because it has no buttons, and the intended service action name is v2.
...
Room cleaning—including natural dirty-room wording—must collect a preferred time before one service alert is created. A clock preference must be later than the current Bangkok minute when requested for today, fall within 10:30–19:29 on a Tuesday–Sunday operating day and retain any explicit future-date context across turns. Invalid preferences stay pending and create no alert; one valid correction creates one alert. `now` and `ASAP` remain valid immediate requests. The response must say that availability may vary and must not tell the guest to submit the request manually. Routine call fallback appears only while housekeeping is open. Direct natural booking intent—including want, would-like, wanna, contraction and take-me/us forms—and **Book with Us** actions must preserve the service category and enter the structured flow; information-only questions must not create alerts. Diving, fishing, snorkeling, taxi, taxi/longtail boat, ferry and motorbike taxi each collect exactly one missing field per turn, retain every supplied valid field, acknowledge side questions/preferences without promising unconfirmed availability, and return to the same next missing field. Shared Bangkok date validation must distinguish valid, past and unparseable input and explain rejected values. Finite choices use buttons; Fun Diving certification accepts useful sanitized human-readable agency/level descriptions; the usable international contact remains last. No booking is confirmed until availability is checked and payment is received. Ferry workflows must never ask for passport data.

Luggage, cleaning and booking are protected operations. If their server request fails, the browser must state that nothing was sent and preserve only the state needed for a safe retry. A completed booking whose protected delivery failed is `delivery_failed`, not active collection: an unrelated next intent must route normally and must never resend the old request. Only an explicit booking-retry command may reuse its completed snapshot, transient contact and existing alert record. Never replace a protected failure with a local success-like answer. Guest success wording is allowed only when at least one intended WhatsApp delivery was accepted.
...
Deploy v5.11.23 with all existing production mappings and secrets unchanged and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. Run only the short changed-function smoke matrix in `DEVELOPMENT_HANDOFF_v5.11.23.md`: booking date/side-question/certification, property issue isolation, forced-open urgent owner operations and the protected controlled-test rotation reset. Stop after reporting the result. The next planned development milestone is v5.11.24 — full visual polish; do not begin it without a new explicit scope. Quick-action activation remains a separate future change; never map service v1, and use `META_STAFF_QUICK_ACTIONS_v5.11.20.md` only after every intended template is Active.
