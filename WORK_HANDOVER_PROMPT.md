...
Deploy v5.11.29 with all existing production mappings, languages, BODY schemas, routes and secrets unchanged, `EXPLORE_ENABLED=false` and `WHATSAPP_STAFF_ACTIONS_ENABLED=false`. Run the concise production smoke check in `DEVELOPMENT_HANDOFF_v5.11.29.md`, including verified/incomplete-registration menu consistency, open/closed-hours generic contact, stale-context isolation and a **lost key** check that stops at the fee prompt. Do not accept the fee or release a spare key merely for testing. Quick-action activation remains a separate future change; never map service v1, and use `META_STAFF_QUICK_ACTIONS_v5.11.20.md` only after every intended template is Active. No subsequent functional or visual milestone is authorized automatically.
